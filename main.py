...
async def admin_users_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    banned_count_result = await db.execute_query_one("SELECT COUNT(*) as count FROM users WHERE is_banned = 1")
    banned_count = banned_count_result['count'] if banned_count_result else 0
...
async def main():
    ...
    if application.job_queue:
        ...
        application.job_queue.run_repeating(
            lambda context: asyncio.create_task(db.cleanup_old_data()),
            interval=3600,
            first=10
        )
        ...
        # إعادة تعيين Rate Limiting يومياً
        application.job_queue.run_repeating(
            lambda context: db.rate_limit_data.clear(),
            interval=86400,
            first=20
        )
        ...
        # التحقق من المحادثات المنتهية الصلاحية
        application.job_queue.run_repeating(
            lambda context: asyncio.create_task(conv_manager.check_timeouts(application)),
            interval=60,
            first=30
        )
